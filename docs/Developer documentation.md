...
| File                      | Description                                                                                                                                                         |
|---------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `arguments.hpp`           | `ProgramArguments` class, parsing, validation, builder class.                                                                                                       |
| `synchronize.hpp`         | Entrypoint for synchronization. Creates a mono- or bi-directional context and respective synchronizer.                                                              |
| `synchronize_one_way.hpp` | one-way synchronization, copying files from the source directory to the target one. Classes `MonodirectionalContext` and `MonodirectionalSynchronizer`.             |
| `synchronize_two_way.hpp` | two-way synchronization, syncing files symmetrically across two directories. Classes `BidirectionalContext` and `BidirectionalSynchronizer`.                        |
...
High-level functions are on the left (generated by Doxygen).

![Function call graph](./function-call-graph.png)

The following picture demonstrates a UML class and function diagram,
a graphical interpretation of the class connectedness (generated by PlantUML).
...
One-way synchronization is run by the `MonodirectionalSynchronizer.synchronize` function.
...
in the target directory tree and a decision is made whether and how to synchronize.
See private helper member functions.
...
source files are copied to the target tree, overridden with newer versions or copied
to different filenames. The deciding factor is
...
a function `MonodirectionalSynchronizer::delete_extra_target_entries` is called
on every common directory.

## Two-way synchronization

Similar to one-way synchronization, directories are synchronized recursively
by `BidirectionalSycnhronizer::synchronize()`,
